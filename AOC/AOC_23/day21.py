...
position = [(r,c) for r, row in enumerate(grid) for c, char in enumerate(row) if char == "S"]
sr,sc = position[0]
...
def fill(sr,sc, ss):
    ...
    while q:
        ...
        for nr, nc in [(r + 1, c), (r - 1, c), (r, c + 1), (r, c - 1)]:
            if nr < 0 or nr >= len(grid) or nc < 0 or nc >= len(grid[0]) or (nr, nc) in seen or grid[nr][nc] == "#":
                continue
            seen.add((nr,nc))
            q.append((nr, nc, s-1))
    return len(ans)
...
print(
    ...
    grid_width * (large_tr + large_bl + large_br + large_tl)
)